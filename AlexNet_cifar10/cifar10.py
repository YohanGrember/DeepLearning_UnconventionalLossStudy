...
tf.app.flags.DEFINE_string('loss', 'entropy',
                           """loss choosed.""")
...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    ...
    biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.02))
  ...
  with tf.variable_scope('local3') as scope:
    ...
    biases = _variable_on_cpu('biases', [384], tf.constant_initializer(0.02))
  ...
  with tf.variable_scope('local4') as scope:
    weights = _variable_with_weight_decay('weights', shape=[384, 192],
                                          stddev=0.04, wd=0.004)
    biases = _variable_on_cpu('biases', [192], tf.constant_initializer(0.02))
  ...
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [192, NUM_CLASSES],
                                          stddev=1/192.0, wd=0.1)
    biases = _variable_on_cpu('biases', [NUM_CLASSES],
                              tf.constant_initializer(0.02)) # put to 0 for some loss
    if FLAGS.loss == 'largemargin':
      softmax_linear = tf.add(large_margin_scores(local4,weights), biases, name=scope.name)
    else:
...
def loss(logits, labels):
  ...
  labels2=tf.one_hot(indices=labels,depth=10,on_value=1.0,off_value=0.0,axis=-1)
