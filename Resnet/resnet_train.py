...
#                tf.summary.histogram(var.op.name + '/gradients', grad)
    ...
    for x in range(FLAGS.max_steps + 1):
        ...
        if step > 1 and step % 100 == 0:
            print("Processing Validation error on %d samples" %(int(1025/FLAGS.batch_size)*FLAGS.batch_size) )
            top1_error_value = mean([sess.run([val_op, top1_error], { is_training: False })[1] for _ in range(int(1025/FLAGS.batch_size))])
